...
- leave a `output/predict.py` that runs as `python output/predict.py <features.csv> <predictions.csv>` and
  writes a CSV with columns `{id_column},prediction` for every row of the features file. The features file
  has the same columns as the training data, minus anything that would not be known at prediction time.
"""
...
def write_holdout(hidden: Path, features: pd.DataFrame, id_column: str, target: pd.Series) -> None:
