...
you write the step-by-step plan that the Executor will carry out with pandas and scikit-learn, one script
per step.
...
Constraints:
- At most 8 steps. Each step becomes one script that later steps build on through files, so merge small
  actions into one step.
- The last step must produce every deliverable the task asks for (files, their names and formats).
