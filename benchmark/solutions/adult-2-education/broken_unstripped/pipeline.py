"""Broken: keeps the space after each separator, so no value equals '?'."""
...
df = pd.read_csv("data/census.data", header=None, names=COLUMNS)
known = df[df["occupation"] != "?"]
share = (known["income"] == " >50K").groupby(known["education"].str.strip()).mean()
answer = {
